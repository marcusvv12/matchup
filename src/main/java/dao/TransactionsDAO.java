...
public class TransactionsDAO extends DAO {
    ...
    public void insert(Transactions transaction) throws SQLException {
        String query = "INSERT INTO matchup.transaction (date, buyer_cpf, products_id, products_user_id, amount_sold, price) " +
                       "VALUES (?, ?, ?, ?, ?, ?)";
    ...
    public Transactions get(int id) throws SQLException {
        String query = "SELECT * FROM matchup.transaction WHERE id = ?";
        try (PreparedStatement preparedStatement = conexao.prepareStatement(query)) {
    ...
    public void update(Transactions transaction) throws SQLException {
        String query = "UPDATE matchup.transaction SET date = ?, buyer_cpf = ?, products_id = ?, products_user_id = ?, " +
    ...
    public void delete(int id) throws SQLException {
        String query = "DELETE FROM matchup.transaction WHERE id = ?";
        try (PreparedStatement preparedStatement = conexao.prepareStatement(query)) {
            preparedStatement.setInt(1, id);
            preparedStatement.executeUpdate();
        }
    ...
    public List<Transactions> getAll(int id) {
        ...
        try {
            String sql = "SELECT * FROM matchup.transaction WHERE products_user_id = ?";
    ...
    public List<Transactions> getProducts(int productId, int user_id) {
        ...
        try {
            String sql = "SELECT * FROM matchup.transaction WHERE products_user_id = ? AND products_id = ?";
